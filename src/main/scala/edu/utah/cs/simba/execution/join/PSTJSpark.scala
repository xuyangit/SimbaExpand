...
case class PSTJSpark(left_key: Expression, right_key: Expression, l: Literal, text_key: Expression, s: Literal,
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    left_partitioned.zipPartitions(right_dup_partitioned) {(leftIter, rightIter) =>
      ...
      left_data ++= left_info.map(x =>
        (x._1, TextualUtil.getText(text_key, left.output, x._2), x._2)
      )
      right_data ++= right_info.map(x =>
        (x._1, TextualUtil.getText(text_key, left.output ++ right.output, x._2), x._2)
      )
