...
case class MBR(low: Point, high: Point) extends Shape {
  ...
  def expand(dis: Double): MBR = {
    val newLow = Point(this.low.coord.map(x => x - dis))
    val newHigh = Point(this.high.coord.map(x => x + dis))
    new MBR(newLow,newHigh)
  }

  def overLap(other: MBR): MBR = {
    require(low.coord.length == other.low.coord.length)
    val array_low = new Array[Double](low.coord.length)
    val array_high = new Array[Double](low.coord.length)
    for (i <- low.coord.indices) {
      val t_low = low.coord(i)
      val o_low = other.low.coord(i)
      val t_high = high.coord(i)
      val o_high = other.high.coord(i)
      array_low(i) = if(t_low > o_low) t_low else o_low
      array_high(i) = if(t_high > o_high) o_high else t_high
     }
    new MBR(Point(array_low), Point(array_high))
  }
